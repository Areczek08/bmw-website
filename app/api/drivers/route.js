...
export async function GET(req) {
  try {
    ...
    return await dbSession(async (db) => {
      let driversQuery = `
        SELECT 
          u.id, u.name, u.firstName, u.discordNick, 
          IF(u.image IS NOT NULL AND u.image != '', 
             IF(u.image LIKE 'http%' OR u.image LIKE '/%', u.image, CONCAT('/api/user/', u.id, '/avatar')), 
             NULL) as image,
          u.role, u.driverStatus, 
      ...
      const driversRaw = await db.all(driversQuery, params);
      ...
      if (trailerIds.length > 0) {
        ...
        trailers = await db.all(`SELECT id, type, plate FROM Trailer WHERE id IN (${placeholders})`, trailerIds);
      }
      ...
      const jobsThisMonth = await db.all(`
      ...
      const driversWithStats = drivers.map(driver => {
        ...
        return {
          ...driver,
          image: driver.image || getSafeAvatarUrl(driver),
